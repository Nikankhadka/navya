...
3. Configure Google auth if web Google sign-in is part of the test pass.
4. Add `navya://auth/callback` as an allowed redirect URL.
5. For local web testing, also add `http://localhost:8081/auth/callback`.
6. Add the stable Vercel preview callback URL: `https://YOUR_PREVIEW_HOST/auth/callback`.
7. Add the production callback URL: `https://YOUR_PRODUCTION_HOST/auth/callback`.
8. If you use a different local web host or port, add that exact `/auth/callback` URL too.
...
## Step 8A: Configure Vercel

1. Create a Vercel project from this repo.
2. Keep deployment on the static Expo web export path generated by `npm run export:web`.
3. Set preview env vars:
   - `EXPO_PUBLIC_SUPABASE_URL`
   - `EXPO_PUBLIC_SUPABASE_ANON_KEY`
   - `EXPO_PUBLIC_APP_ENV=preview`
   - `EXPO_PUBLIC_ENABLE_DEMO_MODE=false`
4. Set production env vars with the same keys, but `EXPO_PUBLIC_APP_ENV=production`.
5. Make sure the production and stable preview hosts are the same URLs already allow-listed in Supabase Auth.
6. Confirm `vercel.json` is present so Vercel serves clean URLs for exported routes.

## Step 9: Manual Test Flow
...
8. Open Nutrition and confirm:
   - seeded meals appear
   - adding a meal works
   - deleting a meal works
9. Open Coach and confirm:
...
10. Open Profile and confirm:
   ...
   - sign out works
11. Repeat the auth flow with Google on web if Google is configured.
...
- Google auth depends on provider setup in Supabase.
- Hosted preview and production should not rely on demo-mode fallback.
